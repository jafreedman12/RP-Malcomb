...
### Authors: Jacob Freedman, Alitzel Villanueva, Maja Cannavo, Emma Clinton, Nick Nonnenmacher, Drew An-Pham

### Original reproduction by Kufre Udoh, Joseph Holler, and Middlebury College Spring 2019 Geography 323 class
...
private_r = here("data","raw","private") #create link to private repository
public_r = here("data","raw","public") #create link to public repository

#Get data downloaded from zip files, load into folder structre in RP-Malcolm folder
...
# Download DHS data once you have logged in to the DHS account
...
```{r r 2010 adaptive capacity data}
...
ta_capacity_2010 = dhshh_2010 %>%
  ...
  filter(!HHID %in% rmv_2010) %>% # ! indicates logical negation (NOT)
  filter(!is.na(ta_id)) %>% 
  ...
  rowwise %>%  # group input by rows, compute on a data frame a row-at-a-time
  mutate(hhlivestock = sum(HV246A, HV246D, HV246E, HV246G, na.rm = T)) %>% #sum all livestock, remove nulls
  ungroup %>%  #ungroup the calculations with the rowwise function
  # using percent_rank(), those  
  # in cases where desc() is used, having a greater value before ranked makes a household more vulnerable, rank everything as quintiles??
  mutate(
...
#ta = mutate(ta, capacity_2010 = capacity_2010 * 40)
# 256 features 
...
```{r LHZ: process livelihood zone data in R}
#Read in LHZ Sensitivity Data
lhz_scores = read_csv(here(private_r, "lhz_toJoinInQ.csv"))

#Join livelihood sensitivity data to lhz zones
# joining livelihood sensitivty and lhz zones, remove LHZ zones with no data with right join
lhz_sensitivity <- lhz %>%
  right_join(lhz_scores, by = "LZCODE")

```

```{r LHZ: calculate livehood sensitivty scores}

lhz_sensitivity <- lhz_sensitivity %>% 
  #rank everything as quintiles??
  mutate(
    ...
    new_disasterCoping = percent_rank(desc(disasterCoping)) * 4 + 1,
  ) %>%
  # calculating capacity score based on table 2 in malcomb et al 
  rowwise %>% 
  mutate(
    livelihoodSensitivity = sum(
      ...
      wageIncome * 0.06,
      cashcropIncome * 0.04,
      new_disasterCoping * 0.04) 
  ) %>%  
  ungroup #%>% 
  #mutate(livelihoodSensitivity_score = livelihoodSensitivity *20)
```


```{r reading rasters into r}
# UNEP layers
#read_stars: read raster in to rstudio
...
#extent of the raster layers/extent that our map will be in
b = st_bbox(
  c(
...
blank[[1]][] = NA  #subset twice, once for the flood raster and one for the drought
...
#warp (rasample) grid from stars objects into new grid
...
#st_clip: clip raster to polygon extent. Building the st_clip function
st_clip = function(x,y) st_intersection(x, st_union(st_geometry(y)))

st_agr(ta) = "constant"

#Apply st_clip function to ta
ta_2010 = st_clip(st_transform(filter(ta, is.na(capacity_2010) == F), 3395), st_buffer(st_transform(lhz, 3395), .01)) %>%
  st_transform(4326)
# 222 features 

# RASTERIZE: making capacity rasters
ta_capacity = st_rasterize(ta_2010[, 'capacity_2010'], blank)
lhz_sensitivity_raster = st_rasterize(lhz_sensitivity[,'livelihoodSensitivity'], blank) 
ta_capacity_2 <- ta_capacity + lhz_sensitivity_raster
```

```{r function to calculate vulnerability}
#this is like a raster calculator function to combine the multiple raster layers
vulnerability = function(geo) {
  # creating mask layer, if we don't have TA capacity in a spot, have data not show up (like on lake)
  mask = geo 
  mask[mask > 0] = 1
  mask[mask == 0] = NA
  
  # masking flood and drought 
  flood = fl * mask * 4  #Flood data is already in classes of 1-5, the multiplying by 4 here normalizes everything to 20 (like in the Malcomb review)
  drought = dr * mask
  
  # reclassifying drought layer into categories of 1-5
  #use [[]] to reference items in a list that have a list
  qt = quantile(drought[[1]], probs = seq(0, 1, 0.2), na.rm = T) #go from 0-1 at intervals of 0.2 (5 intervals))
  ...
  drought = drought %>%
    ...
    ) %>% select(recoded) * 4 #the multiplying by 4 here normalizes everything to 20 (like in the Malcomb review)
  
  # final output (adding component rasters)
  final = ((60-geo) * 0.6 + (drought*0.2 + flood * 0.2)) #Should this be subtraction?? Normalize everything to appropriate values
}
...
ta_final = vulnerability(ta_capacity_2)

ta_2010$vuln = aggregate(ta_final,ta_2010,mean)$capacity_2010
```

```{r misc. map features}
#convert to sf data type, create file called "lakes," containing lakes of malawi
...
ea = lhz %>% #ea data used to find park boundaries and combine with lake data
...
vuln_map = ggplot() +
  ...
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(35.81, 42.41), # c(1.508731, 8.162132), 
    labels = c("Lower Vulnerability", "Higher Vulnerability"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(35.81, 42.41) #c(1.508731, 8.162132)  
  ) +
  scale_x_continuous(breaks = c(33,34,35,36)) +
...
```{r comparing discrete choropleth maps}
...
diffadaptive = ggplot() +
  ...
  scale_x_discrete(breaks = c(33,34,35,36)) +
  labs(title = "Difference in adaptive capacity between Malcomb et. al (2014) and Replicatio") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
```{r comparing continuous raster maps}
orfig5vect = 
  read_sf(here("data", "derived", "public", "georeferencing.gpkg"), 
...
diffrastermap = ggplot() +
  ...
  scale_fill_gradient2(
    ...
    mid = "#f7f7f7",
    high = "#67a9cf",
    breaks = c(-0.98, 0, 0.736),
    labels = c("Diverging 1", "Matching", "Diverging 2"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(-0.98, 0.736)) +
  scale_x_continuous(breaks = c(33,34,35,36)) +
  labs(title = "Difference in Vulnerability maps between Malcomb et. al (2014) and Reproduction") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
```{r saving comparison maps}

save(diffadaptive, diffrastermap, file = here("results","maps","maps.Rdata"))

ggsave(
  here("results","maps","difference_ac_2010.png"),
  plot = diffadaptive,
  width = 8.5,
  height = 11,
  units = "in"
...
ggsave(
  here("results","maps","difference_vulnerability.png"),
  plot = diffrastermap,
  width = 8.5,
  height = 11,
  units = "in"
